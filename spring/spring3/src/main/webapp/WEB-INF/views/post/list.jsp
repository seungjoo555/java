...
<div class="container">
	...
	<form action="<c:url value="/post/list"/>" method="get" id="searchForm">
		...
		<div class="row mb-4">
			<a class="btn btn-outline-primary col-2" href="<c:url value="/post/insert"/>">글쓰기</a>
			<select class="form-control offset-6 col-4" name="order">
				<option value="bo_num" <c:if test="${pm.cri.order == 'bo_num'}">selected</c:if>>최신순</option>
				<option value="bo_view" <c:if test="${pm.cri.order == 'bo_view'}">selected</c:if>>조회순</option>
			</select>
		</div>
	</form>
