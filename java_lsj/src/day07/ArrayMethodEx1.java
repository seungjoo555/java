...
public class ArrayMethodEx1 {
	...
	public static void main(String[] args) {
		//1 ~ 45 사이의 중복되지 않은 랜덤한 수 6개를 만드는 코드를 작성하세요.(메서드를 이용)
		int arr[] = new int[6];
		int min = 1, max = 45;
		...
		//1 ~ 45 사이의 중복되지 않은 랜덤한 수 6개를 입력하는 코드를 작성하세요.(메서드 이용)
		Scanner scan = new Scanner(System.in);
		int user[] = new int [6];
		int userCount = 0;
		while(userCount < user.length) {
			System.out.print("입력 : ");
			int num = scan.nextInt();
			//중복 확인
			if(!contains(user, userCount, num) && (num >= min && num <= max)) {
				//중복되지 않으면 배열에 저장
				user[userCount] = num;
				//저장된 숫자의 개수를 1증가
				userCount++;
	...
	/* 기능 : min ~ max 사이의 중복되지 않은 배열 크기의 랜덤한 수를 배열에 만들어 주고 생성 여부를 알려주는 메서드
	 * 매개변수 : 최솟값, 최대값, 배열 => int min, int max, in []arr
	 * 리턴타입 : 배열의 생성 여부 =>  boolean
	 * 메서드명 : createRandomArrry
	 */
