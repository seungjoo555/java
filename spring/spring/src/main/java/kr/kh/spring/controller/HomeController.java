...
public class HomeController {
	...
	public String signupPost(Model model,MemberVO member) {
		if(memberService.insertMember(member)) {
			...
			model.addAttribute("msg", "회원가입을 못했습니다.");
	...
	public String loginPost(Model model, LoginDTO loginDto) {
		...
		if(user != null) {
			...
			model.addAttribute("msg", "로그인 못했습니다.");
	...
	@PostMapping("/mypage")
	public String mypagePost(Model model, MemberVO member,
			HttpSession session) {
		...
		if(res) {
			...
			model.addAttribute("msg", "회원 정보를 수정하지 못했다.");
			model.addAttribute("url", "/mypage");
		}
		//세션에 회원 정보 수정
