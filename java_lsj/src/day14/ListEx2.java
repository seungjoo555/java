...
public class ListEx2 {
	...
	public static void main(String[] args) {
		...
		ArrayList<Integer> list = new ArrayList<Integer>();	//List의 구현클래스 ArrayList 배열 만들기
		...
		System.out.print("정수 5개 : ");	//프린트 문구
		//5번 반복하기 위해 반복문을 작성
		//i는 0부터 4까지 5번 반복
		for(int i = 0; i < count; i++) {	//정수 5개를 받기 위한 반복문
			//scan.nextInt()를 통해 입력 버퍼에 있는 값 중 정수 값을 가져옴.
			//만약 입력 버퍼에 정수가 아닌 문자나 문자열이 있으면 예외가 발생
			//가저온 정수를 tmp에 저장
			int tmp = scan.nextInt();	//tmp에 콘솔에서 입력한 정수 임시저장
			//Collection에서 제공하는 add 메서드를 이용하여 리스트에 정수값을 추가
			//int인 tmp가 Interger로 *박싱*이 되면서 리스트에 추가
			list.add(tmp);	//리스트에 임시저장한 정수 추가
		}
		//리스트에 있는 정수들을 반복문으로 가져와서 tmp에 저장
		//가져온 Integer의 인스턴스를 int형으로 *언박싱*을 함
		for(int tmp : list) {	//향상된 for문으로 list에 있는 인스턴스를 순서대로 tmp에 대입
			//표준 출력(콘솔창)에 정수를 출력
			System.out.print(tmp + " ");	//프린트 문구
		}
		scan.close();
	}
