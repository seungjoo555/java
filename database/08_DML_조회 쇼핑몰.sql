...
SELECT 
    ca_name,
    if(COUNT(pr_ca_num) > 0, COUNT(pr_ca_num), '등록된 제품 없음') AS 제품수
FROM
    product
        RIGHT JOIN
    category ON pr_ca_num = ca_num
...
SELECT 
    me_id as '회원아이디',
    ifnull(sum(or_total_price), 0) as '누적 주문 금액'
FROM
    `order`
        right JOIN
    member ON me_id = or_me_id
GROUP BY me_id;

# 회원별 등급을 조회하는 쿼리. 등급은 기본이 브론즈, 누적금액이 5만원이상이면 실버, 8만원이상이면 골드
# case문 활용
SELECT 
    me_id as '회원아이디',
    case
