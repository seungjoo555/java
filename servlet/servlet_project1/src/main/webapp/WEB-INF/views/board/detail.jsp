		...
		<c:when test="${board != null}">
			...
			<div>
				<div class="mb-3 mt-3">
				    ...
				    <input type="text" class="form-control" id="community" name="community" value="${board.community.co_name}" readonly>
		  		</div>
				<div class="mb-3 mt-3">
				    <label for="title" class="form-label">제목:</label>
				    <input type="text" class="form-control" id="title" name="title" value="${board.bo_title}" readonly>
		  		</div>
		  		<div class="mb-3 mt-3">
				    <label for="writer" class="form-label">작성자:</label>
				    <input type="text" class="form-control" id="writer" name="writer" value="${board.bo_me_id}" readonly>
		  		</div>
		  		<div class="mb-3 mt-3">
...
<!-- 댓글 삭제 기능 -->
<script type="text/javascript">
//이벤트를 등록할때 요소가 있으면 해당 요소에 이벤트를 등록. 요소가 나중에 추가되면 동작하지 않음
$("선택자").click(function(){})
//document객체에 이벤트를 등록하기 때문에 요소가 나중에 추가되어도 동작함
...
$(document).on("click",".btn-comment-update",function(){
	...
	let btn = 
	`
	<button class="btn btn-outline-primary btn-complete" data-num="\${num}">수정완료</button>
	`;
