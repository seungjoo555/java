...
- 조회(상영시간)
  ...
  - 사용자는 영화, 극장, 날짜를 선택하면 극장 정보(극장 이름, 전체좌석수)와
	상영시간 별 현재 예매 가능 좌석수를 조회할 수 있다
  
- 조회(영화 상세 조회)
  - 사용자는 영화 상세 정보를 조회할 수 있다
  - 상세 정보는 감독, 배우, 장르, 기본정보, 개봉일, 트레일러, 스틸컷이 있다
  - 장르는 등록된 장르를 보여주며, 장르는 여러가지일 수 있다
  - 기본정보에는 관람 나이정보, 런닝 시간, 제작국가가 있다
...
- 감독/배우 관리
  ...
  - 감독/배우는 등록된 인물과 역할, 대표사진을 선택하여 등록한다
...
- 극장 관리
  ...
  - 극장은 지역, 이름, 상영관 정보, 주소, 총 관수를 입력하고, 총 관수
	총좌석수는 등록된 상영관에 맞게 계산이 된다
  - 지역은 등록된 지역 중 하나를 선택한다
  - 상영관 정보는 극장등록과 함께 등록하거나 극장 등록 후 등록할 수 있다
...
- 상영관 관리
  ...
  - 좌석 정보는 행의 수를 입력하고, 각행의 열의 수를 입력한다
...
릴레이션 스키마
  - 인물(인물번호(PK), 이름, 생년월일, 내용, 국가(FK))
  - 감독(등록된 인물(PK,FK감독번호) 역할, 대표사진)
  
  - 장르(장르번호(PK), 장르명)
  - 연령(연령(PK), 연령명(전체관람,청소년,성인))
  - 국가(국가번호(PK), 나라명)
  - 영화 상세정보(영화번호(PK),
			장르(FK, 등록 장르중 여러개 가능), 기본정보, 개봉일, 내용,
            스틸컷(여러개 가능), 트레일러(여러개 가능))
  - 영화 기본정보(영화번호(PK,FK), 제목, 관람가능연령(FK, 등록 연령중 하나), 러닝타임(분으로 입력), 제작국가(FK, 등록된 국가중 여러개 가능)
  - 영화 참여 영화인(대리키(PK), 영화번호(FK), 인물번호(FK))
  - 장르 통합(대리키(PK) 영화번호(FK), 장르번호(FK))
*/
