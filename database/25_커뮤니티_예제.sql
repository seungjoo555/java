...
VALUES((SELECT CO_NUM FROM COMMUNITY WHERE CO_NAME = '자유'), 'abc123', '테스트', '테스트 입니다');
...
# 업로드된 첨부파일은 /2024/02/14/파일명 으로 저장
INSERT INTO BOARD(BO_CO_NUM, BO_ME_ID, BO_TITLE, BO_CONTENT)
...
SELECT * FROM BOARD JOIN COMMUNITY ON CO_NUM = BO_CO_NUM WHERE CO_NAME = '공지';
...
SELECT * FROM RECOMMEND WHERE RE_ME_ID = 'qwe123' AND RE_BO_NUM = 1;
...
UPDATE RECOMMEND SET RE_STATE = 0 WHERE RE_BO_NUM = 1 AND RE_ME_ID = 'qwe123';
# 2-2-2. 추천한 기록이 추천이 아닌 경우 => 비추천이거나 추천/비추천을 취소한 경우에서 다시 추천한 경우
UPDATE RECOMMEND SET RE_STATE = 1 WHERE RE_BO_NUM = 1 AND RE_ME_ID = 'qwe123';
...
BEGIN
    ...
    ELSE
        ...
        CASE
        WHEN _STATE = _RE_STATE THEN
			SET _NEW_STATE = 0;
        # 2-2-2. 추천 상태가 _STATE와 다르면 _STATE로 수정
		WHEN _STATE <> _RE_STATE THEN
			SET _NEW_STATE = _STATE;
		END CASE;
        UPDATE RECOMMEND SET RE_STATE = _NEW_STATE WHERE RE_NUM = _RE_NUM;
    END IF;
END //
DELIMITER ;
...
FROM
    BOARD
WHERE
    BO_CO_NUM = (SELECT CO_NUM FROM COMMUNITY WHERE CO_NAME = '공지')
ORDER BY BO_NUM DESC
LIMIT 0, 10;

# 3번 게시글을 상세조회하는 쿼리
UPDATE BOARD SET BO_VIEW = BO_BIEW + 1 WHERE BO_NUM = 3;
SELECT * FROM BOARD WHERE BO_NUM = 3;

# abc123회원이 3번게시글에 확인했습니다 라고 댓글을달았을 때 쿼리
insert into comment(cm_bo_num, cm_me_id, cm_content)
values(3, 'abc123', '확인했습니다');

# 3번 게시글에 등록된 댓글 1페이지를 조회하는 쿼리
select * from comment where cm_bo_num = 3 order by cm_num desc limit 0, 10;
...
VALUES('admin', '기타', 'comment', '그냥 신고함', 1);
...
('qwe123', '기타', 'board', '내용없음', 1),
# admin회원이 1번 게시글을 '기타', '내용없음' 으로 신고
('admin', '기타', 'board', '내용없음', 1);
# 관리자가 1번 게시글 신고 내역을 모두 '신고처리'로 처리
update report set rp_state = '신고처리' 
where rp_target = 1 and rp_table = 'board' and rp_state = '신고접수';
select count(*) from report where rp_target = 1 and rp_table = 'board' and rp_state = '신고처리';
# 신고처리된 게시글의 신고수를 수정
UPDATE board 
SET 
